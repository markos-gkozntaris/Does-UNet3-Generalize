...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.double_conv = nn.Sequential(
            ...
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False),     # currently padding = valid (original paper has no padding)
...
class Conv_BN_ReLU(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_bn = nn.Sequential(
            # set padding = 0 for dimensions exactly the same with the original
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),     # currently padding = valid (original paper has no padding)
            # batchnorm probably not on original paper, both 2015 papers so yeah ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),     # default stride is kernel_size
        )
